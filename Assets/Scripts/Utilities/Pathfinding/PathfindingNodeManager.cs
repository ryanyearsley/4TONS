...
public class PathfindingNodeManager : MonoBehaviour, IGameManager {
	...
	public void InitializeManager (GameContext gameContext) {
		...
		GameManager.instance.onPortalEnteredEvent += OnPortalEntered;
	}
	...
	public void OnPortalEntered (int nextFloorIndex) {
		Debug.Log ("updating pathfinding nodes. floor index: " + nextFloorIndex);
		currentMapDetails = LevelManager.instance.GetMapDetails (nextFloorIndex);
		StartCoroutine (GenerateFloorNodes (currentMapDetails));
	}
	#endregion
